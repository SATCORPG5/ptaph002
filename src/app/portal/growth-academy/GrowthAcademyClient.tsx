...
import { BookOpen, Trophy, Zap, ChevronRight, Lock, Check, Star } from 'lucide-react';
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        <SectionHeader
          eyebrow="training"
          heading="Growth Academy"
          description="Learn, compete, and level up your stream"
        />
          ...
          {MODULES.map(mod => {
            ...
            return (
              <motion.div key={mod.id} variants={fadeUp}>
                <PortalCard className={`p-5 transition-all ${mod.locked ? 'opacity-60' : 'hover:border-amber-500/20'}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-bold text-foreground/25 tabular-nums">{mod.completed}/{mod.lessons} complete</span>
                    {!mod.locked && (
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-4 max-w-2xl">
          {CHALLENGES.map(ch => (
            <motion.div key={ch.id} variants={fadeUp}>
              <PortalCard className={`p-6 transition-all ${ch.submitted ? 'border-portal-accent/15 bg-portal-accent/[0.03]' : 'hover:border-amber-500/15'}`}>
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  {!ch.submitted ? (
                    ...
                    <span className="text-[9px] font-black text-portal-accent uppercase tracking-wider">Submitted ✓</span>
                  )}
                </div>
              </PortalCard>
            </motion.div>
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-2xl space-y-2">
          {LEADERBOARD.map(entry => (
            <motion.div key={entry.rank} variants={fadeUp}>
              <PortalCard className={`flex items-center gap-4 p-4 transition-all ${
                entry.rank === 1 ? 'border-amber-500/25 bg-amber-500/[0.04]' : 'hover:border-foreground/[0.1]'
              }`}>
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black flex-shrink-0 ${
                  ...
                  {entry.rank === 1 ? <Trophy size={14} className="text-amber-400" /> : <span className="tabular-nums">{entry.rank}</span>}
                </div>
                <div className="flex-1">
                  <p className="text-sm font-black text-foreground">{entry.creator}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[9px] font-bold text-amber-400 tabular-nums">{entry.xp} XP</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {TIPS.map((tip, i) => (
              <motion.div key={i} variants={fadeUp}>
                <PortalCard className={`p-5 cursor-pointer group transition-all hover:border-foreground/[0.12] ${
                  tip.featured ? 'border-amber-500/15 bg-amber-500/[0.03]' : ''
                }`}>
                  {tip.featured && (
