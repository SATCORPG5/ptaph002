...
const TYPE_COLORS: Record<string, string> = {
  ...
  module: 'text-[#14B8A6] bg-[#14B8A6]/10 border-[#14B8A6]/20',
        ...
        <motion.div variants={stagger} initial="hidden" animate="show">
            ...
            <div className="flex gap-2 flex-wrap">
              {niches.map(n => (
                <button key={n} onClick={() => setFilter(n)}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${filter === n ? 'bg-[#14B8A6]/15 text-[#14B8A6] border border-[#14B8A6]/20' : 'text-foreground/25 hover:text-foreground/50 bg-foreground/[0.03] border border-transparent'}`}>
                  {n}
                </button>
              ))}
            </div>
            <button onClick={() => setShowPostRequest(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#14B8A6]/10 border border-[#14B8A6]/20 text-[#14B8A6] text-xs font-black hover:bg-[#14B8A6]/20 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {COLLAB_REQUESTS
              ...
              .map(req => (
              <motion.div key={req.id} variants={fadeUp} className="rounded-2xl border border-foreground/[0.06] bg-foreground/[0.02] p-5 hover:border-[#14B8A6]/20 transition-all group">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#14B8A6]/10 border border-[#14B8A6]/20 flex items-center justify-center">
                      <span className="text-xs font-black text-[#14B8A6]">{req.creator.charAt(0)}</span>
                    </div>
                    <div>
                  ...
                  <div className={`px-2 py-0.5 rounded-full border text-[8px] font-black uppercase ${req.open ? 'bg-[#14B8A6]/10 text-[#14B8A6] border-[#14B8A6]/20' : 'bg-foreground/5 text-foreground/25 border-foreground/10'}`}>
                ...
                {req.open && (
                  <button className="w-full py-2 rounded-xl border border-[#14B8A6]/20 bg-[#14B8A6]/5 text-[#14B8A6] text-xs font-black hover:bg-[#14B8A6]/15 transition-all">
                    Express Interest
                  </button>
                )}
              </motion.div>
            ...
            <motion.div key={battle.id} variants={fadeUp} className={`rounded-2xl border p-5 hover:bg-foreground/[0.02] transition-all ${battle.status === 'live' ? 'border-red-500/20 bg-red-500/[0.03]' : 'border-foreground/[0.06] bg-foreground/[0.02]'}`}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-[10px] text-foreground/30 mt-1">{battle.time} · {battle.viewers} viewers</p>
                </div>
              ...
              <div className="flex gap-3">
                ...
                {battle.status === 'upcoming' && (
                  <button className="flex-1 py-2 rounded-xl bg-[#14B8A6]/10 border border-[#14B8A6]/20 text-[#14B8A6] text-xs font-black hover:bg-[#14B8A6]/20 transition-all">
                    Register to Compete
                  </button>
                )}
              </div>
          ...
          {CALENDAR_EVENTS.map(event => (
            <motion.div key={event.id} variants={fadeUp}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[9px] text-foreground/25 mt-0.5">{event.dept} · {event.date} at {event.time}</p>
              </div>
              <button
          ...
          <div className="rounded-2xl border border-foreground/[0.06] bg-foreground/[0.02] p-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className="w-full bg-[#0F1623] border border-white/15 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#14B8A6]/40 [color-scheme:dark]">
              ...
              <button className="w-full py-3.5 rounded-xl bg-[#14B8A6]/10 border border-[#14B8A6]/20 text-[#14B8A6] text-sm font-black hover:bg-[#14B8A6]/20 transition-all">
                Find My Match
              </button>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <button className="w-full py-3 rounded-xl bg-[#14B8A6]/10 border border-[#14B8A6]/20 text-[#14B8A6] font-black hover:bg-[#14B8A6]/20 transition-all">
                  Post Request
                </button>
              </div>
            </motion.div>
