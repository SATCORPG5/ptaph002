...
import { Users, Swords, Heart, Plus, X, Filter, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        <SectionHeader
          eyebrow="community"
          heading="Collab Lounge"
          description="Discover collabs, battles, and events"
        />
      ...
      {activeTab === 0 && (
        <motion.div variants={stagger} initial="hidden" animate="show">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {COLLAB_REQUESTS.filter(r => filter === 'All' || r.niche === filter).map(req => (
              <motion.div key={req.id} variants={fadeUp}>
                <PortalCard className="p-5 hover:border-portal-accent/20 group">
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-4">
          {BATTLES.map(battle => (
            <motion.div key={battle.id} variants={fadeUp}>
              <PortalCard className={`p-5 hover:bg-foreground/[0.02] ${battle.status === 'live' ? 'border-red-500/20 bg-red-500/[0.03]' : ''}`}>
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-3 max-w-2xl">
          ...
          {CALENDAR_EVENTS.map(event => (
            <motion.div key={event.id} variants={fadeUp}>
              <PortalCard className="flex items-center gap-4 p-4 hover:border-foreground/[0.1]">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border ${TYPE_COLORS[event.type] || 'bg-foreground/5 border-foreground/10 text-foreground/30'}`}>
                  ...
                  {(event.type === 'meeting' || event.type === 'karaoke') && <Users size={16} />}
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className="w-full bg-[#0F1623] border border-white/15 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-portal-accent/40 [color-scheme:dark]">
                  ...
                  <option>Gaming</option><option>IRL</option><option>Just Chatting</option><option>Music</option>
                </select>
              </div>
              <div>
                ...
                <select className="w-full bg-[#0F1623] border border-white/15 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-portal-accent/40 [color-scheme:dark]">
                  ...
                  <option>Duo Stream</option><option>Battle</option><option>React Together</option><option>Talk Show</option><option>Tournament</option>
                </select>
              </div>
              <div>
                ...
                <select className="w-full bg-[#0F1623] border border-white/15 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-portal-accent/40 [color-scheme:dark]">
                  ...
                  <option>TikTok LIVE</option><option>Twitch</option><option>YouTube Live</option>
                </select>
              </div>
