...
export async function signInAction(handle: string, password: string) {
  ...
  console.log(`[signInAction] Attempting login for "${cleanHandle}"`);
  console.log(`[signInAction] Normalized: ${normalizedHandle}`);
  console.log(`[signInAction] Total creators fetched: ${creators.length}`);
  
  if (creators.length > 0) {
    console.log(`[signInAction] First few creators:`, creators.slice(0, 3).map(c => c.handle));
  }
