...
- Type sizes are very small (`text-[9px]`, `text-[10px]`) and weights are uniformly black — visually flat, no clear hierarchy. *(Phase 3 target)*
...
- Search/notifications/avatar are bespoke per portal. Recruiters and admin look like a different product. *(Phase 3 target — `CommandPalette` primitive ready to wire in)*
- No motion language — animations are ad-hoc `duration: 0.15` per component. *(Phase 3/5 target — `TacticalDivider` scan-line + motion tokens established)*
...
| Animation jank on low-end Android | `MotionConfig reducedMotion="user"` + `will-change` only on transforming elements |
