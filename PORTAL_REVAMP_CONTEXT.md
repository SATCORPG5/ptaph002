...
> **⚠️ As-built correction (Phase 4b, 2026-05-28):** `CrmClient` replaced with a **DataTable + Sheet** architecture. `DataTable` primitive extended with `onRowClick` and `focusedRowIndex` props (adds `data-kb-focused` to the matching `<tr>` for keyboard-nav highlight). **nuqs v2** requires `NuqsAdapter` from `nuqs/adapters/next/app` — added to root `layout.tsx` wrapping `ThemeProvider`. URL params: `q` (search), `tier` (filter, `null` = all), `sort` (sort key). `useOptimistic` dispatches tier changes via a `startTransition` wrapper; new `updateCreatorTier` server action updates the `creators` table in Supabase and calls `revalidatePath('/crm')`. `TierChip` is a standalone Michroma badge with a `Tooltip` from `@/components/shadcn-ui/tooltip` — tooltip must override default colors (`bg-background-elevated border border-border text-foreground`) since no `--color-popover` token exists. `Sheet` (side="right") uses the same className override pattern as other shadcn overlays. Full list: [PORTAL_REVAMP_EXECUTION.md](PORTAL_REVAMP_EXECUTION.md) PR #5.
...
| `nuqs` | ~~4~~ 4b ✅ | URL-synced filter state (`q`, `tier`, `sort`) in CRM. `NuqsAdapter` from `nuqs/adapters/next/app` required in root `layout.tsx` for nuqs v2. |
